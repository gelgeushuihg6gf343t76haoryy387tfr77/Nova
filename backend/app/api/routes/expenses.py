...
from app.models import CategoryKind, Expense
from app.schemas.common import MessageResponse
from app.schemas.transactions import ExpenseCreate, ExpenseRead, ExpenseUpdate
from app.services.category_match import expense_text_for_match, resolve_category_id
...
def list_expenses(
    ...
    if created_after is not None:
        stmt = stmt.where(Expense.created_at >= created_after)
    if created_before is not None:
        stmt = stmt.where(Expense.created_at < created_before)
    stmt = stmt.order_by(Expense.occurred_on.desc())
...
def create_expense(
    ...
    text = expense_text_for_match(data.get("vendor"), data.get("notes"))
    data["category_id"] = resolve_category_id(
        db, business_id, CategoryKind.expense, text, data.get("category_id")
    )
    ...
    item = create_entity(db, Expense, {**data, "business_id": business_id})
    logger.info("expense_created id=%s business_id=%s amount_cents=%s currency=%s", item.id, business_id, item.amount_cents, item.original_currency)
    warning = free_plan_soft_warning(db, current_user, business_id)
    return ExpenseRead(**ExpenseRead.model_validate(item).model_dump(exclude={"warning"}), warning=warning)


@router.put("/{expense_id}", response_model=ExpenseRead)
def update_expense(expense_id: uuid.UUID, payload: ExpenseUpdate, business_id=Depends(get_current_business_id), db: Session = Depends(get_db)):
    item = get_owned_entity(db, Expense, expense_id, business_id)
    item = update_entity(db, item, payload.model_dump(exclude_unset=True))
    logger.info("expense_updated id=%s business_id=%s", expense_id, business_id)
    return ExpenseRead.model_validate(item)


@router.delete("/{expense_id}", response_model=MessageResponse)
def delete_expense(expense_id: uuid.UUID, business_id=Depends(get_current_business_id), db: Session = Depends(get_db)):
    item = get_owned_entity(db, Expense, expense_id, business_id)
    logger.info("expense_deleted id=%s business_id=%s", expense_id, business_id)
